...
Эта заметка рассказывает о том, как устроена геоинформационная система [Кижского заказника](https://vodlozero.nextgis.com). Заметка также описывает то, как создавалась система, как собираются полевые данные и другие ее функции.
...
* Частичное наличие сотового сигнала и интернета.
...
* [NextGIS Toolbox](https://toolbox.nextgis.com) - набор дополнительных инструментов для разных операций (ссылки на конкретные инструменты - в тексте)
...
В качестве иллюстрации --- типовое сообщение в телеграм:
...
Веб ГИС - это база пространственных данных, размещенная на сервере. Веб ГИС обеспечивает единообразные механизмы доступа и обмена данными. Доступ к материалам, размещенным в Веб ГИС, можно осуществлять из настольных, мобильных программ и через браузер. Веб ГИС развернута на основе **NextGIS Web**.

![webgis-structure.png](webgis-structure.png "Структура данных перенесенная в NextGIS Web")
...
![webmap.png](webmap.png "Веб-карта в NextGIS Web содержащая все слои данных")
...
При работе офлайн можно смотреть и контуры объектов, и всю атрибутивную информацию по каждому контуру.

![mobile-vector.jpg](mobile-vector.jpg "Векторный слой границ лесоустроительных выделов 2004 г.")

![mobile-raster.jpg](mobile-raster.jpg "Растровый слой границ лесоустроительных выделов 2004 г. раскрашенный согласно условным обозначениям")

![mobile-attributes.jpg](mobile-attributes.jpg "Атрибутивная информация по одному из выделов")

## Координация маршрутов обследований

Перед выходом в поле координатор выбирал нужные участки для обследования и отмечал их в специальном слое на Веб-карте.

![webmap-routes.png](webmap-routes.png "Намеченный маршрут на Веб-карте")
...
![mobile-route.jpg](mobile-route.jpg "Тот же маршрут после синхронизации данных в NextGIS Mobile + треки и сделанные описания")

## Внесение экспресс-описаний и находок видов

![mobile-express.jpg](mobile-express.jpg "Точки с кратким описанием и фотографиями")

## Фотографии на карте

Помимо фотографий прикладываемых к объектам на карте у команды накопились и "обычные" фотографии, сделанные обычной камерой и хранящиеся в памяти телефона. Если это выставлено в настройках операционной системы, у таких фотографий появляются координаты, они прописываются в специальных тегах в каждой фотографии (т.н. EXIF-теги).

Такие отдельно сделанные фотографии с координатами, прописанными в них, были превращены в слой данных с помощью инструмента [Фото с EXIF в слой Веб ГИС](https://toolbox.nextgis.com/operation/exif2resource).

Если в настройках телефон забыли выставить разрешение прописывать координаты или они не были сделаны по какой-то еще причине --- не беда. Фотографии, у которых не было координат в EXIF, но было точное время съемки, были привязаны к карте с помощью треков описанных выше и инструмента [Добавление координат к фотографиям](https://toolbox.nextgis.com/t/gpx2exif).
...
![qgis-transitions.png](qgis-transitions.png "Пример расчета переходов между биотопами 1886 и 2004 гг.")
...
Можно [**написать в телеграм**](https://t.me/answer42geo/231) или оставить вопрос в форме ниже.
